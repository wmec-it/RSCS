...
pub fn msstore(program_id: &str) -> std::process::Output {
    let install_command: String = format!(
        "winget install -e --source msstore --id {} --silent --disable-interactivity --accept-package-agreements --accept-source-agreements",
        &program_id
    );
    let install_command_output = std::process::Command::new("powershell")
        .arg("-Command")
...
pub fn winget(program_id: &str) -> std::process::Output {
    let install_command: String = format!(
        "winget install -e --source winget --id {} --silent --disable-interactivity --accept-package-agreements --accept-source-agreements",
        &program_id
    );
